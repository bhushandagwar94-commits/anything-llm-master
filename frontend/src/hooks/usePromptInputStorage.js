...
export function clearPromptInputDraft(storageKey) {
  try {
    ...
    [localStorage, sessionStorage].forEach((storage) => {
      const map = safeJsonParse(storage.getItem(USER_PROMPT_INPUT_MAP), {});
      if (map[storageKey]) {
        map[storageKey] = "";
        storage.setItem(USER_PROMPT_INPUT_MAP, JSON.stringify(map));
      }
    });
  } catch {}
}
...
export default function usePromptInputStorage({ promptInput, setPromptInput }) {
  ...
  const storage = isPublic ? sessionStorage : localStorage;

  useEffect(() => {
    const serializedPromptInputMap =
      storage.getItem(USER_PROMPT_INPUT_MAP) || "{}";
