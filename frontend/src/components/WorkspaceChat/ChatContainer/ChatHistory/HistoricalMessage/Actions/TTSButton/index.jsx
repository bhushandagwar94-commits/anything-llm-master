...
export default function TTSMessage({ slug, chatId, message }) {
  const { settings, provider, loading, disabled } = useTTSProvider();
  // Public Chat Mode: TTSProvider is not mounted — disable all TTS UI silently.
  if (disabled || !chatId || loading) return null;
