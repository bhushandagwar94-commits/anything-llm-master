...
export default function RenderMetrics({ metrics = {} }) {
  ...
  return (
    <button
      ...
      className={`border-none flex items-center gap-x-2 px-3 py-1 rounded-full bg-white/5 light:bg-black/5 border border-white/5 light:border-black/5 backdrop-blur-md transition-all duration-300 ${showMetricsAutomatically ? "opacity-100 translate-y-0" : "opacity-0 translate-y-1 md:group-hover:opacity-100 md:group-hover:translate-y-0"}`}
    >
      <div className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse" />
      <p className="text-[10px] font-bold uppercase tracking-widest text-white/40 light:text-slate-500">
