  ...
  return (
    <MessageActionsProvider>
      <ThoughtExpansionProvider>
        <div
          className={`chat-history-container markdown text-white/80 light:text-theme-text-primary font-light ${textSizeClass} flex-1 min-h-0 overflow-y-auto pb-8 pt-6 md:pt-2 md:mx-0 flex flex-col items-center justify-start ${showScrollbar ? "show-scrollbar" : "no-scroll"}`}
          id="chat-history"
          ref={chatHistoryRef}
          onScroll={handleScroll}
        >
          <div className="w-full max-w-[980px] flex flex-col gap-y-6 px-4 md:px-0">
...
function buildMessages({
  ...
  return history.reduce((acc, props, index) => {
    ...
    const sanitizedContent = isAssistant ? sanitizeAssistantOutput(props.content) : props.content;
    const isLastBotReply =
      index === history.length - 1 && isAssistant;
