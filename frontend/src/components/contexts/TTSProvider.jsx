 ...
 */
/**
 * Default fallback context returned when useTTSProvider is called outside
 * a TTSProvider (e.g. in Public Chat Mode). This prevents runtime crashes
 * while disabling all TTS functionality cleanly.
 */
const TTS_DISABLED_CONTEXT = {
  settings: {},
  provider: "native",
  loading: false,
  disabled: true,
...
export function useTTSProvider() {
  ...
  // In Public Chat Mode, TTSProvider is not mounted. Return a safe no-op
  // context rather than crashing so TTS components can self-disable.
  if (!context) return TTS_DISABLED_CONTEXT;
  return context;
}
