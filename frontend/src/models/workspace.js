...
const Workspace = {
  ...
  getParsedFiles: async function (slug, threadSlug = null) {
    ...
    if (!response.ok) {
      ...
      console.error("[API ERROR] Parsed files fetch failed:", errText);
      return { success: false, error: errText, files: [], contextWindow: 0, currentContextTokenCount: 0 };
    }
    ...
    try {
      ...
      console.error("[WORKSPACE JSON ERROR]", text);
      return {
        success: false,
