...
function systemEndpoints(app) {
  ...
  app.get("/system/branding-config", async (_, response) => {
    try {
      const config = await SystemSettings.getBrandingConfig();
    ...
    async (request, response) => {
      try {
        ...
        await SystemSettings._updateSettings({
          seetech_chat_heading: config.heading,
          seetech_chat_subtitle: config.subtitle,
          seetech_welcome_message: config.welcome_message,
          seetech_chat_suggested_prompts: config.suggested_prompts,
          seetech_chat_placeholder: config.placeholder,
          seetech_primary_color: config.primaryColor,
          seetech_bg_primary: config.bgPrimary,
          seetech_typography: config.typography,
        });

        const updatedConfig = await GlobalBrandingManager.saveBranding(config);
        if (!updatedConfig) return response.sendStatus(500).end();
        response.status(200).json({ success: true, config: updatedConfig });
      } catch (e) {
        console.error(e.message, e);
        response.sendStatus(500).end();
      }
  ...
  app.get(
    ...
    async (_, response) => {
      try {
        const online = await new CollectorApi().online().catch(() => false);
        response.status(200).json({ online });
      } catch (e) {
        console.error(e.message, e);
        response.status(200).json({ online: false });
      }
    }
  );
  ...
  app.post(
    ...
    async (request, response) => {
      try {
        const { testLLM } = require("../utils/helpers");
        const body = reqBody(request);
        const { success, error } = await testLLM(body);
