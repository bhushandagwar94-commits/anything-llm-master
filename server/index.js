...
const { configEndpoints } = require("./endpoints/config");
const { llmConfigEndpoints } = require("./endpoints/llmConfigs");
const { GlobalProviderOrchestrator } = require("./utils/llm/ProviderOrchestrator");
const { GlobalHealthMonitor } = require("./utils/llm/ProviderHealthMonitor");

// Initialize LLM Orchestrator
GlobalProviderOrchestrator.initialize().then(() => {
  GlobalHealthMonitor.start();
}).catch(e => {
  console.error("[CRITICAL] Failed to initialize AI Provider Orchestrator:", e.message);
});
...
configEndpoints(apiRouter);
llmConfigEndpoints(apiRouter);
