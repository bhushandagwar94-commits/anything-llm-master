...
async function chatSync({
  ...
  const greetingResponse = await handleGreetingOverride({
    workspace,
    message,
    user,
    thread,
  ...
  const messages = await LLMConnector.compressMessages(
    {
      systemPrompt: await chatPrompt(workspace, user, thread, sessionId, message),
      userPrompt: message,
      contextTexts,
      chatHistory,
      attachments,
  ...
  const cleanedText = Sanitizer.cleanFullResponse(textResponse);
...
async function streamChat({
  ...
  if (completeText?.length > 0) {
    const cleanedText = Sanitizer.cleanFullResponse(completeText);
    const { chat } = await WorkspaceChats.new({
      workspaceId: workspace.id,
      prompt: message,
      response: {
