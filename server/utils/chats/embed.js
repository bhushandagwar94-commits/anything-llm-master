...
async function streamChatWithForEmbed(
  ...
  console.log("\x1b[36m[PUBLIC AI] Initializing IIOS Orchestration Engine...\x1b[0m");
  console.log(`\x1b[34m[WORKSPACE CONFIG]\x1b[0m Workspace: ${embed.workspace?.name} | Slug: ${embed.workspace?.slug}`);
  console.log(`\x1b[34m[MODEL]\x1b[0m Model: ${modelOverride ?? embed.workspace?.chatModel ?? "System Default"}`);
  ...
  // Inject any parsed files for this session (using sessionId as thread slug)
  const { WorkspaceParsedFiles } = require("../../models/workspaceParsedFiles");
  const { WorkspaceThread } = require("../../models/workspaceThread");
  const thread = await WorkspaceThread.get({
    slug: String(sessionId),
    workspace_id: embed.workspace.id,
  });
  ...
  const messages = await LLMConnector.compressMessages(
    {
      systemPrompt: await chatPrompt(embed.workspace, username, null, sessionId, message),
      userPrompt: message,
      contextTexts,
      chatHistory,
      attachments,
  ...
  await EmbedChats.new({
    embedId: embed.id,
    prompt: message,
    response: { text: completeText, type: chatMode, sources, metrics, attachments },
