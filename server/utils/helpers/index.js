 ...
 */
function getLLMProvider({ provider = null, model = null, _bypassOrchestrator = false } = {}) {
  const { GlobalProviderRouter } = require("../providers/ProviderRouter");
  const LLMSelection = provider ?? process.env.LLM_PROVIDER ?? "openai";
  
  if (!_bypassOrchestrator) {
    try {
      const managed = GlobalProviderRouter.getProvider(LLMSelection, model);
  ...
  try {
    switch (LLMSelection) {
      ...
      default:
        console.warn(`[PROVIDER ERROR] LLM provider "${LLMSelection}" is invalid. Falling back to SafeLLM.`);
        return new SafeLLM(embedder);
    }
  } catch (e) {
    console.error(`[CRITICAL PROVIDER FAILURE] Failed to initialize provider "${LLMSelection}": ${e.message}`);
    return new SafeLLM(embedder);
  }
}

/**
 * A safe fallback LLM provider that prevents runtime crashes when no valid 
 * provider is found. Returns professional "Service Unavailable" messages.
 */
class SafeLLM {
  ...
  isValidChatCompletionModel() { return true; }
  constructPrompt() { return ""; }
  async compressMessages(args) { return args.chatHistory || []; }
  async getChatCompletion() {
    return {
      textResponse: "I apologize, but my intelligence core is currently being reconfigured. Please check back in a few moments.",
  ...
  async streamGetChatCompletion() {
    const chunk = {
      choices: [{
        delta: { content: "I apologize, but my intelligence core is currently being reconfigured. Please check back in a few moments." },
        finish_reason: "stop"
      }]
    };
    
    // Create an async iterable that behaves like an OpenAI stream
    const stream = {
      [Symbol.asyncIterator]: async function* () {
        yield chunk;
      },
      start: Date.now(),
      metrics: { completion_tokens: 0, prompt_tokens: 0, total_tokens: 0, outputTps: 0, duration: 0 },
      endMeasurement: function() { return this.metrics; }
    };
    return stream;
  }
  async handleStream(response, stream, responseProps) {
    const { handleDefaultStreamResponseV2 } = require("./chat/responses");
    return await handleDefaultStreamResponseV2(response, stream, responseProps);
  }
}
 ...
 */
function getEmbeddingEngineSelection({ provider = null, _bypassOrchestrator = false } = {}) {
  const { GlobalProviderRouter } = require("../providers/ProviderRouter");
  const { NativeEmbedder } = require("../EmbeddingEngines/native");
  const engineSelection = provider ?? process.env.EMBEDDING_ENGINE ?? "native";
  
  if (!_bypassOrchestrator) {
    try {
      const managed = GlobalProviderRouter.getEmbedder(engineSelection);
      if (managed) return managed;
    } catch (e) {
      console.error(`[EMBEDDING ROUTER ERROR] ${e.message}`);
    }
  }
...
/**
 * Re-initializes LLM providers by clearing any cached instances and potentially
 * triggering downstream updates. In this architecture, it ensures future requests
 * pick up the latest environment variables.
 */
function reloadLLMProviders() {
  try {
    console.log("[LLM RELOAD] Triggering global provider re-initialization...");
    const { GlobalProviderOrchestrator } = require("../llm/ProviderOrchestrator");
    GlobalProviderOrchestrator.reload();
    return { success: true };
  } catch (e) {
    console.error("[LLM ERROR] Failed to reload providers:", e.message);
    return { success: false, error: e.message };
  }
}

/**
 * Validates a provider configuration by attempting a basic handshake or model list.
 * @param {Object} config - The provider configuration to test
 * @returns {Promise<{success: boolean, error: string | null}>}
 */
async function testLLM(config = {}) {
  const { LLMProvider, ...rest } = config;
  const providerSelection = LLMProvider ?? process.env.LLM_PROVIDER;

  try {
    console.log(`[LLM CONFIG] Testing connection for provider: ${providerSelection}`);
    const provider = getLLMProvider({ provider: providerSelection });

    // Try a simple chat completion with a tiny prompt to verify connectivity
    ...
    if (response && response.textResponse) {
      console.log(`[LLM CONFIG] Connection successful for ${providerSelection}`);
      return { success: true };
    }

    throw new Error("Provider returned an empty or invalid response.");
  } catch (e) {
    console.error(`[LLM ERROR] Connection test failed for ${providerSelection}:`, e.message);
    return { success: false, error: e.message };
  }
}
