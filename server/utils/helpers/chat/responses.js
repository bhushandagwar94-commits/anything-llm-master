  ...
  return new Promise(async (resolve) => {
    ...
    try {
      for await (const chunk of stream) {
        ...
        if (token) {
          console.log(`\x1b[36m[RAW LLM RESPONSE]\x1b[0m "${token}"`); // DEBUG LOG
          fullText += token;
...
function writeResponseChunk(response, data) {
  ...
  if (data?.type === "textResponseChunk" && typeof data.textResponse === "string") {
    data.textResponse = Sanitizer.sanitize(data.textResponse);
  }
