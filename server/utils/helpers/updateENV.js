...
async function updateENV(newENVs = {}, force = false, userId = null) {
  ...
  console.log(`[LLM CONFIG] Processing environment update for ${ENV_KEYS.length} keys.`);
  ...
  for (const key of ENV_KEYS) {
    ...
    // Log the change (masking potential sensitive values if not a simple provider/model choice)
    const isSensitive = !["LLMProvider", "LLMModel", "EmbeddingEngine", "VectorDB"].includes(key);
    console.log(`[LLM CONFIG] Updating ${key} (${envKey}): ${isSensitive ? "********" : nextValue}`);
    ...
    // exit early.
    if (errors.length > 0) {
      console.error(`[LLM ERROR] Validation failed for ${key}:`, errors.join(", "));
      error += errors.join("\n");
      break;
    }
    ...
    // exit early.
    if (errors.length > 0) {
      console.error(`[LLM ERROR] Pre-update failed for ${key}:`, errors.join(", "));
      error += errors.join("\n");
      break;
    }
  ...
  if (error.length === 0) {
    const { reloadLLMProviders } = require("./index");
    console.log("[LLM CONFIG] Environment update successful. Triggering reload.");
    reloadLLMProviders();
  } else {
    console.error("[LLM ERROR] Environment update failed with errors.");
  }
