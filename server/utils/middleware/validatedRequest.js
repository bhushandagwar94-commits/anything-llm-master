...
async function validatedRequest(request, response, next) {
  ...
  // Bypass validation for Public Workspace parsing/retrieval if it's a public embed workspace
  const { slug } = request.params;
  if (slug && (request.path.endsWith("/parse") || request.path.endsWith("/parsed-files") || request.path.endsWith("/delete-parsed-files"))) {
    const { EmbedConfig } = require("../../models/embedConfig");
    const { Workspace } = require("../../models/workspace");
    const workspace = await Workspace.get({ slug });
    if (workspace) {
      const hasEmbed = await EmbedConfig.get({ workspace_id: workspace.id });
      if (hasEmbed) {
        next();
        return;
      }
